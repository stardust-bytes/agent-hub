<template>
  <div class="flex flex-col bg-cyber-bg min-w-0">
    <div class="px-3 py-2 border-b border-cyber-border bg-cyber-dark flex items-center justify-between shrink-0">
      <span class="text-cyber-accent text-xs tracking-widest font-mono"><HiTerminal class="w-3 h-3 inline" /> {{ t('artifacts.header') }}</span>
    ...
    <div class="flex-1 overflow-y-auto px-3 py-3 min-h-0">
      ...
      >
        <HiTerminal class="text-cyber-accent/30 w-3 h-3 inline" /> {{ t('artifacts.empty') }}
      </div>
      ...
      <template v-else>
        <div v-for="(block, i) in codeBlocks" :key="`code-${i}`" class="mb-3">
          <div class="bg-cyber-dark border border-cyber-border rounded overflow-hidden">
            ...
            <pre class="px-3 py-2 text-xs text-slate-100 overflow-x-auto leading-relaxed font-mono">{{ block.code }}</pre>
          </div>
        </div>

        <div
          v-if="proseHtml"
          class="text-sm text-slate-100 leading-relaxed font-mono prose-invert"
          v-html="proseHtml"
        />
      </template>
    </div>
...
import { HiTerminal } from 'vue-icons-plus/hi'
