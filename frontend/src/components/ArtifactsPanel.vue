  ...
  <div v-show="visible" class="flex flex-col h-full bg-surface border-l border-border">
    ...
    <div class="flex-1 overflow-y-auto px-3 py-2 space-y-3">
      ...
      <div v-if="fileContent !== null" class="border border-border rounded-lg overflow-hidden">
        <div class="bg-muted px-3 py-1.5 text-sm text-muted-foreground font-mono border-b border-border">
          📄 {{ fileName }}
        </div>
        <pre class="text-sm text-foreground font-mono p-3 whitespace-pre-wrap break-all max-h-60 overflow-y-auto bg-muted">{{ fileContent }}</pre>
      </div>
      ...
      <div v-for="plan in plans" :key="plan.id" class="border border-border rounded-lg overflow-hidden">
        ...
        <div class="px-3 py-2 space-y-0.5">
          <div v-for="step in plan.steps" :key="step.id" class="flex gap-2 items-start text-sm">
            <span :class="step.status === 'DONE' ? 'text-success' : step.status === 'DOING' ? 'text-warning' : step.status === 'FAILED' ? 'text-danger' : 'text-muted-foreground'" class="shrink-0 w-6 text-sm leading-5 font-mono">
      ...
      <div v-for="(result, i) in toolResults" :key="i" class="border border-border rounded-lg overflow-hidden">
        <div class="bg-muted px-3 py-1.5 text-sm text-warning font-mono border-b border-border">
          ⚙ {{ result.toolName }}
        </div>
        <pre class="text-sm text-foreground font-mono p-3 whitespace-pre-wrap break-all max-h-40 overflow-y-auto bg-muted">{{ result.content }}</pre>
      </div>
    </div>
  </div>
</template>
