<template>
  <BaseModal :model-value="modelValue" :closable="true" max-height="80vh" @update:model-value="$emit('update:modelValue', $event)">
    <template #header>
      <span class="text-foreground text-sm font-mono">{{ t('tools.config.title') }}: {{ tool?.name }}</span>
    </template>
    <div class="px-3 py-4 space-y-3">
      <div v-for="(schema, key) in schemaProps" :key="key" class="mb-2">
        ...
        <input
          ...
          class="w-full bg-surface px-2 py-1.5 text-sm font-mono text-foreground outline-none"
        />
        <select
          v-else
          v-model="formData[key]"
      ...
      <p v-if="!schemaProps || Object.keys(schemaProps).length === 0" class="text-muted-foreground text-sm font-mono">{{ t('tools.config.noConfig') }}</p>
    </div>
    <template #footer>
      <div class="flex gap-2 justify-end">
        <button @click="$emit('update:modelValue', false)" class="px-3 py-1.5 text-sm font-mono text-muted-foreground bg-surface hover:text-foreground transition-colors duration-150">{{ t('tools.form.cancel') }}</button>
        <button @click="onSave" class="px-3 py-1.5 text-sm font-mono text-primary-foreground bg-primary rounded-lg hover:bg-primary/90 transition-colors duration-150">{{ t('tools.config.save') }}</button>
      </div>
    </template>
  </BaseModal>
</template>
