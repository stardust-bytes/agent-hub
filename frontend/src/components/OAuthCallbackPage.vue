<template>
  <div class="flex-1 flex flex-col items-center justify-center bg-background">
    <div v-if="status === 'loading'" class="text-primary font-mono text-sm">
      {{ t('oauth.callback.loading') }}
    </div>
    <div v-else-if="status === 'success'" class="text-success font-mono text-sm">
      {{ t('oauth.callback.success') }}
    </div>
    <div v-else class="flex flex-col items-center gap-3">
      <div class="text-danger font-mono text-sm">{{ t('oauth.callback.error', { msg: errorMsg }) }}</div>
      <router-link v-if="showBackLink" to="/connectors"
        class="text-primary font-mono text-sm rounded-lg border border-primary/30 px-2.5 py-1 transition-colors duration-150 hover:bg-primary/10">
        {{ t('oauth.callback.back') }}
      </router-link>
      <button v-else @click="handleClose"
        class="text-muted-foreground font-mono text-sm rounded-lg border border-input px-2.5 py-1 transition-colors duration-150 hover:bg-muted hover:text-primary">
