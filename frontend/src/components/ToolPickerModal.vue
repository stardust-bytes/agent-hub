<template>
  ...
  >
    <template #header>
      <span class="text-gray-900 text-sm font-mono">{{ t('agents.toolsHeader') }}</span>
    </template>
    ...
    <div class="px-3 py-2">
      <input
        ...
        class="w-full bg-white text-gray-900 text-sm font-mono border border-gray-300 rounded-md px-2.5 py-1.5 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 mb-2"
        autocomplete="off"
      />
      ...
      <div v-else class="space-y-0.5">
        <label
          ...
          :key="tool.name"
          class="flex items-start gap-2 px-2 py-1.5 cursor-pointer hover:bg-white transition-colors duration-150"
        >
          <input type="checkbox" :value="tool.name" v-model="checked" class="accent-blue-600 mt-0.5 shrink-0" />
          <span class="min-w-0">
            <span class="text-gray-900 text-sm font-mono block">{{ tool.name }}</span>
            <span class="text-gray-500/60 text-sm font-mono block truncate">{{ tool.description }}</span>
          </span>
        </label>
        <div v-if="!filteredTools.length" class="text-gray-500 text-sm font-mono py-2">{{ t('tools.no_tools') }}</div>
      </div>
    </div>

    <template #footer>
      <div class="flex items-center gap-2 justify-end">
        <span class="text-gray-500/60 text-sm font-mono mr-auto">{{ t('agents.toolsSelected', { n: checked.length }) }}</span>
        <button
          type="button"
