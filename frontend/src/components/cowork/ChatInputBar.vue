<template>
  <div class="shrink-0">
    <div class="max-w-60rem mx-auto w-full px-3 pb-3">
      <div class="bg-white border border-gray-300 rounded-md px-3 py-2 focus-within:border-blue-500 focus-within:ring-1 focus-within:ring-blue-500 transition-colors duration-150">
        <form @submit.prevent="onSubmit" class="flex items-center gap-2">
          <div class="relative flex-1">
            <input
              ...
              v-model="input"
              @keydown="onKeydown"
              class="flex-1 bg-transparent text-gray-900 text-sm outline-none font-mono placeholder-gray-400 w-full"
          ...
          <button
            v-if="streaming"
            @click="emit('stop')"
            class="text-gray-500 text-sm px-2 py-0.5 transition-colors duration-150 hover:text-gray-900 shrink-0"
          >{{ t('chat.stop') }}</button>
        </form>
      ...
      <div class="flex items-center justify-between pt-2">
        ...
        <button
          ...
          class="text-blue-600 text-sm px-2 py-0.5 transition-colors duration-150 hover:text-blue-700"
